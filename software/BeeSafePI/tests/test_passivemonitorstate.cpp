...
//#include "../src/monitor/states/PassiveMonitorState.h"
...
/*
    MonitorState *monitorState = new PassiveMonitorState(comms, &testAccount);
    //-----test handleLatLng-----
    std::pair<double, double> latLng1(55.828182,-4.213158);
    MonitorState *success = monitorState->handleLatLng(latLng1);
    //---if takes correct location as outside---
    if (success != nullptr){failures+="latLngCorrect;";}
    std::pair<double, double> latLng2(60.828182,1.213158);
    success = monitorState->handleLatLng(latLng2);
    //---if takes incorrect location as inside---
    if (success == nullptr){failures+="latLngCorrect;";}
*/
    if(failures != ""){
