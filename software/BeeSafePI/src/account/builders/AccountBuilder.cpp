...
Account* AccountBuilder::build()
{
    ...
    web::json::value &jsonContactsValue = root.at(U(ACCOUNT_KEY_CONTACTS));
    ...
    web::json::value &jsonFencesValue = root.at(U(ACCOUNT_KEY_FENCES));
...
{
    return new Contact(
            jsonContact.at(U(CONTACT_KEY_FORENAME)).as_string(),
            jsonContact.at(U(CONTACT_KEY_SURNAME)).as_string(),
            jsonContact.at(U(CONTACT_KEY_NUMBER)).as_string(),
            jsonContact.at(U(CONTACT_KEY_KEY)).as_string()
    );
}
...
{
    return new GeoFence(
            jsonGeoFence.at(U(FENCE_KEY_SAFE)).as_bool(),
...
bool AccountBuilder::isContactStructure(web::json::value &jsonContact)
{
    return !jsonContact.is_null() && jsonContact.is_object()
           && jsonContact.has_string_field(U(CONTACT_KEY_FORENAME))
           && jsonContact.has_string_field(U(CONTACT_KEY_SURNAME))
           && jsonContact.has_string_field(U(CONTACT_KEY_NUMBER))
           && jsonContact.has_string_field(U(CONTACT_KEY_KEY));
}
...
bool AccountBuilder::isGeoFenceStructure(web::json::value &jsonGeoFence)
{
    return !jsonGeoFence.is_null() && jsonGeoFence.is_object()
           && jsonGeoFence.has_boolean_field(U(FENCE_KEY_SAFE))
...
bool AccountBuilder::isGeoPathStructure(web::json::value &jsonGeoPath)
{
    return !jsonGeoPath.is_null() && jsonGeoPath.is_object()
           && jsonGeoPath.has_boolean_field(U(FENCE_KEY_SAFE))
